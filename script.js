    ...
    document.getElementById('contactForm').addEventListener('submit', async (e) => {
      ...
      // Check if Formspree is configured (ID replaced from placeholder)
      const formAction = form.action || '';
      const hasFormspree = formAction.includes('formspree.io/f/') && !formAction.includes('YOUR_FORM_ID');
      ...
      if (hasFormspree) {
        ...
        try {
          ...
          waFallback(name, phone, message);
        }
      } else {
        // --- Formspree not set up yet → send via WhatsApp ---
        waFallback(name, phone, message);
        form.reset();
      }
    });

    function waFallback(name, phone, message) {
      const text = [
        `Hello! I reached out via the website 🌸`,
        name    ? `Name: ${name}`   : '',
        phone   ? `Phone: ${phone}` : '',
        message ? `\n${message}`    : '',
      ].filter(Boolean).join('\n');
